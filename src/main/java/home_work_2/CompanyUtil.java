package home_work_2;


public class CompanyUtil {

    public static Company[] getCompanies() {
        Employee[] employees = EmployeeUtil.getEmployees();

        Company c1 = new Company();
        c1.setEmployees(new Employee[]{employees[0], employees[1]});
        c1.setCompanyName("Company TEst1");

        Company c2 = new Company();
        c2.setEmployees(new Employee[]{employees[2], employees[3]});
        c2.setCompanyName("Company TEst2");

        Company c3 = new Company();
        c3.setEmployees(new Employee[]{employees[4], employees[5]});
        c3.setCompanyName("Company TEst3");
        return new Company[]{c1};
    }

    public static void getMaxSalary() {
        Company[] companies = getCompanies();
        for (int i = 0; i < companies.length; i++) {
            Employee temp = companies[i].getEmployees()[0];
            for (int j = 0; j < companies[i].getEmployees().length; j++) {
                double salary = companies[i].getEmployees()[j].salary;
                if (temp.salary > salary) {
                    temp = companies[i].getEmployees()[j];
                }
            }
            System.out.println(companies[i].getCompanyName() + " " + temp.salary);
        }
    }
}
