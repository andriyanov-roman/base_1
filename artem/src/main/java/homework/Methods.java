...
//                "\n6. Increase the salary for 15%" +
    ...
    public static void start() throws IOException {
        CompanyUtil.getCompanies("artem/src/main/java/additionalFiles/companies");
        do {
            welcome();
            switch (Methods.scanner.next()){
                case "1":
                    for (int i = 0; i < CompanyUtil.companies.length; i++) {
                        ArrayList<Employee> employees = EmployeeUtil.getEmployees(("artem/src/main/java/additionalFiles/companies/" + CompanyUtil.companies[i]), ":");
